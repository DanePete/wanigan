...
if(outputAt>=0&&(!process.argv[outputAt+1]||process.argv[outputAt+1].startsWith('--')))throw new Error('--out requires a directory.');
const out=outputAt>=0?path.resolve(process.argv[outputAt+1]):path.join(root,'docs/visuals/scout-workspace',before?'before':'after');
...
try {
 ...
 if(before){
  ...
  await page.getByRole('button',{name:'Check local inventory',exact:true}).click();await waitText('.scout-feedback','Local preview completed');assert.deepEqual((await calls()).at(-1),['run',{mode:'preview'}]);
  await page.getByRole('button',{name:'Check official sources online',exact:true}).click();await waitText('.scout-feedback','Online check completed');assert.deepEqual((await calls()).at(-1),['run',{mode:'manual',allowNetwork:true}]);assert.equal(await page.evaluate(()=>window.__scoutSettings.networkEnabled),false);
  for(const outcome of ['blocked','failed','running','mystery']){
   await page.evaluate(status=>{window.__scoutRunStatus=status;window.__scoutRunDetail='Fixture recorded outcome: '+status;},outcome);await page.getByRole('button',{name:'Check official sources online',exact:true}).click();await waitText('.scout-feedback',outcome==='running'?'still running':'Fixture recorded outcome: '+outcome);assert.doesNotMatch(await page.locator('.scout-feedback').innerText(),/completed/);
  }
  ...
  await page.getByRole('checkbox',{name:/Enable Scout workspace/}).uncheck();assert(await page.getByRole('button',{name:'Check official sources online',exact:true}).isDisabled());await page.getByRole('checkbox',{name:/Enable Scout workspace/}).check();await waitText('.scout-feedback','Saved watch permissions');
