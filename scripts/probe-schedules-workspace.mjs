...
try{
 ...
 await page.addInitScript(()=>{
  ...
  for(const schedule of window.__schedules)if(schedule.kind==='headless'&&schedule.id!=='s6')Object.assign(schedule.payload,{executionVersion:1,providerId:'claude',providerProfileFingerprint:'fixture-claude'});
  ...
  window.wanigan=new Proxy(api,{get(api,service){
   if(service==='providers')return new Proxy(api.providers,{get(providers,method){if(method==='list')return async()=>(await api.providers.list()).map(provider=>({...provider,profileFingerprint:'fixture-'+provider.id,capabilities:{...provider.capabilities,headlessJson:true,headlessBudget:provider.id!=='codex'}}));return providers[method];}});
 ...
 if(before){await page.locator('.sc-item').first().waitFor();await capture('schedules');await page.locator('.sc-list').scrollIntoViewIfNeeded();await capture('saved-schedules');}
  ...
  await view.getByRole('button',{name:'Scheduler settings',exact:true}).click();await view.getByRole('button',{name:'Run anything due now',exact:true}).click();await view.getByText(/No new occurrences queued/).waitFor();await view.getByRole('button',{name:'Fire schedules while Wanigan is closed',exact:true}).click();await view.getByRole('button',{name:'Stop background scheduler',exact:true}).waitFor();await capture('scheduler-settings');
  await view.locator('.sc-support summary').click();await choose('s2');
