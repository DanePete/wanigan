...
try{
 ...
 if(before){
  ...
  await page.getByRole('navigation',{name:'Work shortcuts'}).getByRole('button',{name:'Changes',exact:true}).click();
  await page.locator('.gt-bar').waitFor();assert.equal(await page.locator('.space-switch-name').innerText(),'platform');
  assert.match(await page.locator('.gt-bar').innerText(),/platform/);
  await page.keyboard.press('Meta+Shift+C');await page.getByRole('heading',{name:'Context',exact:true}).waitFor();
  assert.equal(await page.locator('.space-switch-name').innerText(),'platform');
  await open();await search().fill('all');await page.keyboard.press('Enter');await page.locator('.sessions-view').waitFor();
  checks.push('Space selection scopes Sessions, Changes and Context; All spaces returns project-only views to Sessions.');
