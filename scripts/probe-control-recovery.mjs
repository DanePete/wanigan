...
try {
  ...
  await page.addInitScript(STUB + `
    (() => {
      ...
      const node=(id,kind,title,status,dependsOn)=>({id,docketId:'recovery-fixture',kind,title,status,dependsOn,instructions:kind==='verify'?'Run the saved review commands against the implementation checkout.':title,claimPath:null,providerId:null,model:null,sessionId:null,worktree:null,startedAt:now-90000,endedAt:status==='completed'?now-30000:null,detail:null,deferUntil:null,queued:false});
