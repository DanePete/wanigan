...
if(outFlag!==-1&&(!process.argv[outFlag+1]||process.argv[outFlag+1].startsWith('--')))throw Error('--out requires a directory');
const out=outFlag===-1?path.join(root,'docs/visuals/next-ten',before?'before':'after'):path.resolve(process.argv[outFlag+1]);mkdirSync(out,{recursive:true});
...
try{
 ...
 await page.addInitScript(()=>{
  ...
  window.wanigan=new Proxy(api,{get(target,service){
   ...
   if(service==='companion')return new Proxy(target.companion,{get(obj,key){if(key==='snapshot')return async(...args)=>{const result=await obj.snapshot(...args);if(window.__brokenView)result.models[0].label={fixture:'intentional render failure'};return result;};return obj[key];}});
 ...
 await (before?page:page.getByRole('group',{name:'Session details',exact:true})).getByRole('button',{name:before?'Timeline':'Activity',exact:true}).click();await page.locator('.tl').waitFor();await capture('timeline');
 await (before?page:page.getByRole('group',{name:'Session details',exact:true})).getByRole('button',{name:before?'Code':'Changes',exact:true}).click();await page.locator('.code-panel').waitFor();await capture('code');
 ...
 await page.evaluate(()=>window.__brokenView=true);await go('Meta+k');await page.getByRole('combobox',{name:'Search views, projects, live sessions, settings and archived transcripts'}).fill(before?'Mission':'Home');await page.keyboard.press('Enter');await page.locator(before?'.empty[role="alert"]':'.view-recovery').waitFor();await capture('recovery');await page.evaluate(()=>window.__brokenView=false);await page.getByRole('button',{name:'Reload view',exact:true}).click();await page.locator('.mission-room').waitFor();
 if(!before){
  ...
  await page.getByRole('group',{name:'Session details',exact:true}).getByRole('button',{name:'Activity',exact:true}).click();const timing=page.locator('.tl-summary').filter({has:page.locator('summary').filter({hasText:'Tool timing'})});await timing.waitFor();assert.equal(await timing.getAttribute('open'),null);
  ...
  await timing.locator('summary').click();assert.notEqual(await timing.getAttribute('open'),null);await timing.locator('summary').click();ok('Timeline leads with events and keeps timing and filter recovery available');
  await page.getByRole('group',{name:'Session details',exact:true}).getByRole('button',{name:'Changes',exact:true}).click();await page.locator('.code-file').filter({hasText:'src/checkout.ts'}).click();await page.getByRole('button',{name:'Read code',exact:true}).click();
  ...
  await page.getByRole('group',{name:'Session details',exact:true}).getByRole('button',{name:'Activity',exact:true}).click();await page.getByRole('button',{name:'Back to terminal',exact:true}).click();assert.equal(await draft.inputValue(),'A draft stays with this session.');assert.equal(await terminal.evaluate(el=>el.isConnected&&el.getClientRects().length>0),true);
  ...
  await palette();await search.fill('Home');await page.keyboard.press('Enter');await page.locator('.mission-room').waitFor();await page.evaluate(()=>document.documentElement.dataset.motion='full');await page.waitForFunction(()=>document.querySelector('.mission-presence .wanigan-orb')?.dataset.physics==='ready');
 ...
 assert.deepEqual(errors,[]);writeFileSync(path.join(out,'verification.json'),JSON.stringify({checks,errors,provenance:'Real Electron renderer with authored fixtures, including one deliberately invalid companion model label to exercise the error boundary. No live app session, provider call, or repository action.'},null,2)+'\n');
}catch(error){if(page){await page.screenshot({path:path.join(out,'failure.png')}).catch(()=>{});const recovery=await page.locator('.view-recovery-details').textContent().catch(()=>null);writeFileSync(path.join(out,'failure.json'),JSON.stringify({error:error.message,recovery,checks,errors},null,2)+'\n');console.log('Recovery details:',recovery);console.log(await page.evaluate(()=>['.mission-temperament','.mission-presence','.mission-orb-caption','.mission-room','.body'].map(selector=>{const e=document.querySelector(selector);return e?{selector,rect:e.getBoundingClientRect().toJSON(),scroll:e.scrollTop,positionArea:getComputedStyle(e).positionArea}:null;})).catch(()=>null));}throw error;}
