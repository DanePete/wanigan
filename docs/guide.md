...
The [README](../README.md) is the tour. This is the reference behind it: how each
surface works, what it deliberately refuses to do, and the sharp edges worth
knowing before you rely on it or fork it.

Wanigan is the same job at three speeds — one agent working interactively on one
repo, one prompt fanned across ten thousand rows, and the gap between them, one
...
[Batches](#batches) · [Model capabilities](#model-capabilities-come-from-the-api) ·
[Skills and Context](#skills-and-context) · [Wanigan Compound](#wanigan-compound) ·
[Provider packs](#provider-packs) · [Insights](#insights) · [The API key](#the-api-key) ·
[Six things that will bite you if you fork this](#six-things-that-will-bite-you-if-you-fork-this) ·
[Shape](#shape) · [Keys](#keys) · [What this app believes](#what-this-app-believes) ·
[Preparing a public demo](#preparing-a-public-demo)
...
is automatic. Model-assisted consolidation is connected, and it runs only
through three gates: consent pinned to a profile fingerprint, routing to the
backend that produced the signals on a declared protocol, and metering read
from recorded runs. A harness that reports no usage is refused by name, and an
unpriced call is recorded as unpriced rather than totalled as spend. Even then
it phrases only unauthored nominations where a claim can exist, so no hidden
model call happens. Semantic content can be processed only by the same model
backend that originally saw it; web, MCP, attachment and other excluded
external content is ineligible. Only
...
`provider-pack.json` inside it and choose **Settings → Provider packs → Re-read from disk**.
...
[docs/provider-packs.md](provider-packs.md).
...
Every chord the shell binds lives in one table,
[`src/renderer/src/bindings.ts`](../src/renderer/src/bindings.ts), and the view
routes in [`src/shared/routes.ts`](../src/shared/routes.ts). The in-app cheat
sheet (`?` or `⌘/`) renders the same table, so it is the authoritative list.
The terminal owns its keystrokes: while it has focus, only `⌘.` gets through.
...
| `⌘⌫` | close the active exited session tab |
...
See [the demo privacy audit](demo-privacy.md) for scope and remaining work.
