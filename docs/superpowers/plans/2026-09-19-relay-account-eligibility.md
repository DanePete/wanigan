...
**Unfinished on purpose: two owner links.** A receipt is accounted for when the provider itself answered with an error and its request id (`not-charged-provider-stated`, direct `api.anthropic.com` responses only, since an error relayed by a proxy behind `ANTHROPIC_BASE_URL` is not the provider speaking), when an owning ledger recorded the meters (`metered`), or when a CLI reported a cost (`reported-estimate`). Since `5940e8d` every outcome is bound to its receipt, response and the owner's recorded meters under an evidence hash that Recovery recomputes, so a missing, changed or mismatched owner row accounts for nothing. Each owning ledger is named in [the evidence contract](../../../src/main/paid-operation-evidence.ts) with the columns that are its meters. Four are linked: Improve prompt, the learning CLI, Companion (converted into a module in `ae89fbf`, linked in `0ae1b3f`) and the dry-run sample (moved into a module file in `81482ae`). The dry run kept no ledger at all, so required Usage now owns `usage_direct_requests` for any paid request whose caller keeps none: a closed source label, the model and the reported token counts, never a prompt, reply or price. Two are **not** linked: legacy interviews and batch submission. Both live in unconverted `src/main` files that AGENTS.md requires converting first, and neither is a one-line link. Interviews keep only running totals, so each needs a per-request row before anything can account for it. A batch is billed per result rather than per submission, so its receipt should hand off to the `batches` row that Recovery already inspects separately, which is a design decision and not yet made. Until then their successful requests are recorded as answered with no recorded meters and still refuse a restore. `usage_direct_requests` reaches the Usage screen through the same module contribution every other ledger uses, as "Claude Platform · Dry run": tokens as recorded, and an estimate worked out from them that is never shown as a reported bill. No renderer code changed, so there are no new screenshots.
