...
**Follow-up on `feat/paid-pathway-conversion` (a worktree branched from `5940e8d`).** Companion is converted and linked, in the two commits AGENTS.md requires: `9e5297b` makes it a module (schema, four channels, halt stopper and started-services gate leave `db.ts` and `index.ts`; its egress entry is still in `egress.ts`), and `8f5498e` lets a turn whose input and output meters reached `companion_turns` account for its receipt as `metered`, under the evidence binding Recovery recomputes. Three owner links remain, and none is a mechanical move. Legacy interviews record only a running `calls` and `spend_usd` on the interview row, so there is no per-request row to bind: the link needs an additive per-call ledger first. The dry-run sample still has no ledger. Batch submission meters when results arrive, not when the submission is answered, so its owner evidence is a different shape from a Messages call. Each of those is a schema decision to make before its conversion, not during it.
