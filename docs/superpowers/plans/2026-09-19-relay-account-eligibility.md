...
Legacy interviews are **not** linked. `interview.ts` is an unconverted 532-line surface, and it keeps only running totals, so it needs a per-request row before anything can account for a request, not only a call to the helper. Until then an interview's successful requests are recorded as answered with no recorded meters and still refuse a restore, which the existing legacy-interview reader does on its own anyway. No renderer code changed in any of this, so there are no new screenshots.
