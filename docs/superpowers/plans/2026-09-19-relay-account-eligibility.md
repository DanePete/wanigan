...
**Left open on purpose: Recovery still refuses a restore for every `interviews` row.** The per-call ledger makes it possible to narrow that read to interviews whose `calls` equals their metered ledger rows, but it was not narrowed. An interview that failed an attempt before paid receipts existed and was continued afterwards could match its ledger while an older attempt is recorded nowhere, and wrongly dropping a restore blocker is the dangerous direction. That is the operator's call, not a follow-on edit.

**Two owner links remain, and both wait on one conversion.** The dry-run sample and batch submission live in `src/main/batch/`: 4,324 lines in 17 files, 47 references in `index.ts`, a poller in the ordered halt list, and `batch/pricing` and `batch/anthropic` imported across the modules. The dry run cannot be carved out alone, because its channel is `batch:dryRun` and a module may only own channels under its own id. Converting batch is its own planned piece of work. After it, the dry run needs an owner row (it has no ledger today), and batch submission needs owner evidence shaped around results arriving later rather than around the submission's answer.
