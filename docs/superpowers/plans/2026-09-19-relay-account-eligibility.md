...
**Every paid ledger is now linked.** A receipt is accounted for in one of four ways, each bound since `5940e8d` to its receipt, its response and the owner's recorded evidence under a hash that Recovery recomputes, so a missing, changed or mismatched owner row accounts for nothing. Each owning ledger is named in [the evidence contract](../../../src/main/paid-operation-evidence.ts).

- `not-charged-provider-stated`: the provider itself answered with an error and its request id. Direct `api.anthropic.com` responses only, since an error relayed by a proxy behind `ANTHROPIC_BASE_URL` is not the provider speaking.
- `metered`: an owning ledger recorded the request's tokens. Linked for Improve prompt, for Companion (converted into a module in `ae89fbf`, linked in `0ae1b3f`), and for the dry-run sample (moved in `81482ae`). The dry run kept no ledger at all, so required Usage now owns `usage_direct_requests` for any paid request whose caller keeps none: a closed source label, the model and the reported token counts, never a prompt, reply or price. It reaches the Usage screen as "Claude Platform · Dry run" through the same module contribution every other ledger uses, with an estimate worked out from the tokens that is never shown as a reported bill.
- `reported-estimate`: the learning CLI reported a cost, which its vendor documents as a client-side estimate.
- `recorded-in-batch-ledger`: a batch submission (moved in `7ed1a0a`) whose `batches` row was created from that response. Nothing is metered at submission because a batch bills per result; the exposure is that row's, and Recovery already inspects it for unfinished, un-ingested and lost results. Only the row's immutable columns are bound, because its status legitimately changes as the batch runs.

Interviews were the last (converted into a module in `9d15110`). An interview row keeps only running totals, and a later answer overwrites an earlier one, so it never had a per-request record. Each metered interview request now writes one to `usage_direct_requests`, naming its interview, and shows on the Usage screen as "Claude Platform · Interview". Recovery's interview check narrowed accordingly, from "any interview blocks a restore" to "an interview with a counted call that has no per-request record blocks". **Every interview from before this change still blocks**, because its history really cannot be proven, and so does one whose count was never recorded, one whose reply reported no usage, and one that failed on its first call before paid receipts existed. No renderer code changed in any of this, so there are no new screenshots.

**Latency added to every launch, measured.** An attended Codex launch waits on the status read: about 0.86 s end to end against the real `codex-cli 0.155.1` app-server (initialize 254 ms, `account/read` 6 ms, `account/rateLimits/read` 591 ms, `model/list` 2 ms), reused for 45 seconds when the login is file-witnessed. An attended Claude launch waits on `claude auth status --json`: about 175 ms. One machine, two or three runs each; not a distribution.

**Exercised live, read-only, once.** After implementation the reader's exact five methods were sent to the real Codex app-server on the default account, printing shapes and timings only. Established: the reads succeed **without** `experimentalApi`, so the stable-surface handshake is safe; `usedPercent` is an integer and `resetsAt` is Unix seconds; one page holds the whole five-model list with `nextCursor: null`; `accountId` is populated; and production returns a second bucket, `base_model_inference`, beside `codex`, which the previous reader dropped. That account reported `ordinaryUsageAllowed: false` at the time, so the gate as shipped refuses unattended Codex work on it and tells an attended launch why. Claude's `auth status --json` also carries an `orgId`, which the login digest now uses in place of the editable `orgName`, so a renamed organisation is the same login and a different organisation is not. The id stays in the main process. Still not exercised live: a signed-out or API-key Codex account, keyring credential storage, a second page of models, any real interview, Companion, dry-run or batch request, and the launch gate itself inside a running app against a real account.
