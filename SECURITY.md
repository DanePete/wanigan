...
credential and the random ntfy topic are encrypted at rest through Electron's
OS credential storage. If that storage becomes unavailable or an existing blob
cannot be decrypted, the listener and push sender pause instead of silently
minting replacement credentials. A Tailscale Serve mapping is configured and
removed separately; disabling Wanigan's listener does not remove that proxy.
