...
async function runGate(nodeId: string, trigger: ProofTrigger): Promise<GateRun | { skipped: 'unchanged' }> {
  ...
  try {
    ...
    const summary = passed
      ...
      : commandsPassed
        ? `${run.results.length} review command(s) passed${where}${when}. Current checkout not verified: ${run.freshness.reason}`
        : `Review gate failed after ${run.results.length} command(s)${where}${when}.`;
    const last = commandsPassed ? null : run.results[run.results.length - 1] ?? null;
    ...
    const proof: DocketProof = { id: uid('proof'), docketId: parent.id, nodeId, kind: 'test', status: passed ? 'passed' : commandsPassed ? 'recorded' : 'failed', summary, createdAt: now(), gate };
...
function latestTestProof(docketId: string, nodeId: string): TestProofRow | null {
  // A reopened task needs fresh evidence even when its checkout has not yet changed.
  const reopened = nodeRow(nodeId).reopened_at ?? 0;
  return db().prepare(`SELECT id,status,detail_json FROM work_proofs
    WHERE docket_id=? AND node_id=? AND kind='test' AND created_at >= ? ORDER BY created_at DESC, rowid DESC LIMIT 1`)
    .get(docketId, nodeId, reopened) as TestProofRow | undefined ?? null;
}
...
export async function completeNode(nodeId: string, input: { detail?: string; decision?: 'approve' | 'request_changes' | 'reject' }): Promise<DocketNode> {
  ...
  const implementationGateRequired = node.kind === 'implement' && parent.gate_on_stop === 1;
  const checkedNodes = node.kind === 'verify' || implementationGateRequired ? [node] : verifyNodes;
  ...
  const testsPassed = node.kind === 'verify' || implementationGateRequired ? passes[0] === true
  ...
  return db().transaction(() => {
    ...
    if (implementationGateRequired && decision === 'approve' && !testsPassed) {
      throw new Error(gateRuns.has(nodeId)
        ...
        : 'This goal holds implementation tasks until the review gate passes for the current checkout and commands. The agent stopping is a claim that it is done; run the gate, or let it run when the agent next stops, and complete the task once it passes.');
    }
    ...
    // The reviewer's note stays on the Mac and is handed to a reopened task.
    db().prepare('INSERT INTO work_proofs (id,docket_id,node_id,kind,status,summary,detail_json,created_at) VALUES (?,?,?,?,?,?,?,?)')
      .run(proof.id, proof.docketId, proof.nodeId, proof.kind, proof.status, proof.summary,
        JSON.stringify(node.kind === 'review' ? { decision, note: detail } : {}), proof.createdAt);
    // Persist one outcome per launched phase for the Model evidence surface.
