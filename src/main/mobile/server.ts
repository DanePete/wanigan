...
export function mobileStatus(): MobileMonitorStatus {
  ...
  return {
    ...
    pairingUrl: token && mobileCredentialsReady() ? `${dashboardBase(config)}#token=${encodeURIComponent(token)}` : '',
    pairingCode: token && mobileCredentialsReady() ? pairingCode(token) : '',
    tokenFingerprint: token && mobileCredentialsReady() ? `${token.slice(0, 8)}…${token.slice(-4)}` : '',
...
export async function setMobileConfig(patch: MobileConfigPatch): Promise<MobileMonitorStatus> {
  ...
  if (patch.pushTopic !== undefined && !topicLooksStrong(next.pushTopic)) {
  ...
  if (next.pushTopic !== current.pushTopic) {
    await updateMobileSecrets((secrets) => ({ ...secrets, topic: next.pushTopic }));
  }
...
export async function regenerateMobileToken(): Promise<MobileMonitorStatus> {
  await updateMobileSecrets((secrets) => ({ ...secrets, token: generateToken() }));
  return startMobileMonitor();
}
...
export async function regenerateMobilePushTopic(): Promise<MobileMonitorStatus> {
  await updateMobileSecrets((secrets) => ({ ...secrets, topic: generateTopic() }));
  return startMobileMonitor();
}
 ...
 */
export async function forgetMobilePushDevice(id: string): Promise<MobileMonitorStatus> {
  await forgetPushDevice(id);
  return mobileStatus();
}

export async function forgetAllMobilePushDevices(): Promise<MobileMonitorStatus> {
  await forgetAllPushDevices();
  return mobileStatus();
}
 ...
 */
export async function regenerateMobilePushKeys(): Promise<MobileMonitorStatus> {
  await rotatePushKeys();
  return mobileStatus();
}
...
export async function startMobileMonitor(): Promise<MobileMonitorStatus> {
  ...
  await initializeMobileSecrets();
  // Disabling or stopping while Keychain awaits consent must still win.
  if (requestedVersion !== lifecycleVersion) return mobileStatus();
  config = mobileConfig();
  if (!config.dashboardEnabled) return mobileStatus();
