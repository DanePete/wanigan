...
/**
 * Goals: one goal's contract, the shape of its task graph, and the evidence
 * recorded against it. Two levels — the goals this Mac is carrying, then one of
 * them opened.
 *
 * Reading, and only reading. Starting a task, approving a review, arming
 * unattended dispatch and setting a cap are all decisions with money or a
 * working tree behind them, and every one of them stays at the Mac. The lead
 * paragraph says so in the served markup rather than in a release note, so it is
 * readable before any read succeeds and on a device that has never reached the
 * Mac — and so nobody opens this screen expecting a button that is not coming.
 *
 * The three things this screen exists to get right, all of which the desktop
