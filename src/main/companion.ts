...
type Completion = { text: string; model: string; input: number | null; output: number | null };
type Dependencies = {
  database: typeof db;
...
const SYSTEM = `You are Wanigan, a calm, perceptive companion for a person supervising coding agents.
Be warm, concise and specific. Have a little personality without cute filler.
...
export function parseCompanionAnswer(text: string, sources: CompanionSource[]): { answer: string; sources: CompanionSource[] } {
  ...
  const value = JSON.parse(cleaned) as { answer?: unknown; sourceIds?: unknown };
  ...
  return {
    ...
    async ask(value: unknown): Promise<CompanionTurn> {
      ...
      const snapshot = JSON.stringify({ readAt: facts.readAt, totals: { projects: facts.totalProjects, running: facts.running, needsYou: facts.needsYou },
        ...
        projects: selected.map((p) => ({ ...p, sessions: p.sessions.slice(0, 10) })), sources });
...
export const companion = createCompanionService({
  ...
  complete: async (model, messages, signal) => {
    const response = await client().messages.create({ model, max_tokens: 1_500, system: SYSTEM, messages },
      { signal, timeout: 60_000, maxRetries: 0 });
    return { text: response.content.filter((p) => p.type === 'text').map((p) => p.text).join('\n'), model: response.model,
      input: response.usage?.input_tokens ?? null, output: response.usage?.output_tokens ?? null };
  },
});
