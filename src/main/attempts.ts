...
async function grade(attemptId: string, projectId: string, baseCommit: string, worktree: string | null): Promise<Graded> {
  ...
  try {
    ...
    // A zero exit is historical command evidence. It can grade this attempt
    // only when the checkout and saved commands still match that evidence.
    if (run.status === 'passed' && run.freshness.state !== 'current') {
      return unavailable(`The review commands exited successfully, but this attempt's checkout was not verified: ${run.freshness.reason}`,
        { reviewRunId: run.id, tree: snapshot.tree, oracle });
    }
