...
export async function runNamingTemplateSmoke(check: Check, say: Say, tmp: string): Promise<void> {
  ...
  const wt = await worktrees.createWorktree(repo, project.name, sessionId, undefined, branch);
  try {
    ...
    let unsafe = '';
    try { await worktrees.createWorktree(repo, project.name, 's_unsafe_000003', undefined, 'feature/../../escape'); } catch (e) { unsafe = String(e); }
