...
import { db } from './db';
import { tokenEvidenceForMany, usageForMany } from './otel';
...
export function recordOutcomeReview(node: { id: string; docket_id: string; kind: string },
  ...
  const d = db();
  const sessions = d.prepare(`SELECT attempts.id,s.provider_id,s.model,s.effort,s.backend_id,s.harness_id,s.provider_profile_json,s.started_at,s.ended_at
  ...
  const usages = usageForMany(ids);
  const tokens = tokenEvidenceForMany(ids);
  const codexTokens = codexTokenEvidenceForSessions(ids, at);
  const attempts = sessions.map(row => {
    const usage = usages[row.id];
    const tokenEvidence = codexTokens[row.id] ?? tokens[row.id];
    ...
    return {
      ...
      inputTokens: tokenEvidence.counts.inputTokens, outputTokens: tokenEvidence.counts.outputTokens,
      cacheReadTokens: tokenEvidence.counts.cacheReadTokens, cacheWriteTokens: tokenEvidence.counts.cacheWriteTokens,
      reasoningTokens: tokenEvidence.counts.reasoningTokens, tokenEvidence,
    };
  });
