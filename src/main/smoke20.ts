 ...
 * after an explicit selection and confirmation.
 */
export async function runAttachmentRetentionSmoke(check: Check, say: Say): Promise<void> {
  ...
  try {
    ...
    const refused = attachments.reclaimAttachments({ now, sessionIds: [ids.inert] });
    check(fs.existsSync(inert.storedPath) && refused.filesRemoved === 0,
      'with retention off, even an explicitly selected pass removes nothing', refused);

    attachments.setAttachmentRetention(30);
    const selected = attachments.planAttachmentReclaim({ now }).candidates.filter(item => item.sessionId === ids.inert);
    const reportResult = attachments.reclaimAttachments({ now, sessionIds: selected.map(item => item.sessionId), approved: selected.map(item => ({ sessionId: item.sessionId, fingerprint: item.fingerprint })) });
    const pass = attachments.recordAttachmentReclaim(reportResult);
    check(!!pass && pass.how === 'on-request' && !fs.existsSync(inert.storedPath) && pass.filesRemoved >= 1
      && pass.bytesFreed >= Buffer.byteLength(`staged for ${ids.inert}\n`),
    'a selected, content-pinned pass removes an inert directory past the saved window, counting bytes from files confirmed gone', pass);
    ...
    check(attachments.lastAttachmentReclaim()?.ranAt === pass.ranAt && attachments.lastAttachmentReclaim()?.filesRemoved === pass.filesRemoved,
      'the measured pass remains available through the persisted retention summary');

    const index = appSource('src/main/index.ts');
    check(index.includes("handle('attachmentStorage:reclaim'") && index.includes('attachments.recordAttachmentReclaim(report)')
      && !index.includes('attachments.reclaimAttachmentsIfDue()') && !index.includes("handle('attach:reclaimNow'"),
    'the app records confirmed cleanup without an automatic timer or an unconfirmed legacy deletion channel');
    const settings = appSource('src/renderer/src/views/Settings.tsx');
    check(settings.includes('<AttachmentStorage />') && !settings.includes('This screen cannot yet measure or reclaim it'),
