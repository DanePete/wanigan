...
import { writeExport } from './modules/batch';
import { db } from './db';
...
export async function runSmoke(): Promise<void> {
  ...
  // Run from the built bundle, which is where a relative require has no file to
  // find. Rows are counted by id: a row's output text carries quoted newlines.
  const exportPath = path.join(os.tmpdir(), `wanigan-smoke-export-${process.pid}.csv`);
  const exportedRows = (text: string) => (text.match(/^r\d+-\d+,/gm) ?? []).length;
  let exported = '';
  try {
    writeExport(sub.runId, 'csv', exportPath);
    for (let i = 0; i < 50 && exportedRows(exported) < 3; i++) {
  ...
  check(exported.startsWith('custom_id,row_index,status,') && exportedRows(exported) === 3,
  ...
  try {
    ...
    const { runScoutSmoke } = await import('./smoke5');
    await runScoutSmoke(check, say);
    const { runCheckpointSmoke } = await import('./smoke6');
    await runCheckpointSmoke(check, say);
    const { runCheckpointLaunchSmoke } = await import('./smoke-checkpoint-launch');
    await runCheckpointLaunchSmoke(check, say);
    ...
    const { runCompanionSmoke } = await import('./smoke12');
    await runCompanionSmoke(check, say);
    const { runPreflightSmoke } = await import('./smoke13');
  ...
  say('── suite coverage');
  // Calibrated against 1237 observed on 2026-09-07, with a small margin for
