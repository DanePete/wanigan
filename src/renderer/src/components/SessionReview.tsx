...
import { Note, PageHead, Segmented } from './bits';
...
export default function SessionReview({ session, onClose, onSendToBatch }: {
  ...
  const [section, setSection] = useState<'changes' | 'checks'>('changes');
  return portal(<div {...backdropProps}>
    <section {...dialogProps} className="modal pane session-review" aria-label="Review session work">
      ...
      <div className="session-review-toolbar">
        <Segmented label="Session review section" value={section} onChange={setSection}
          options={[{ value: 'changes', label: 'Changes' }, { value: 'checks', label: 'Checks & evidence' }]} />
      ...
      <div className="session-review-layout">
        {/* Keep both mounted: switching sections must preserve selected files,
            unsaved check commands and reads of a running check. */}
        <section className="session-review-changes" aria-label="Session checkout changes" hidden={section !== 'changes'}>
        ...
        <section className="session-review-checks" aria-label="Session checkout checks" hidden={section !== 'checks'}>
