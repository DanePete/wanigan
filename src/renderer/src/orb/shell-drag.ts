...
export class ShellDrag {
  ...
  constructor(private readonly device:GPUDevice,private readonly liquid:Sim,impacts:GPUBuffer){
    ...
    this.groups=['A','B'].map(side=>device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[
      ...
      {binding:2,resource:{buffer:liquid.buf['vel'+side]}},{binding:3,resource:{buffer:impacts}}]}));
  }
  step(dt:number,angularVelocity:number,vortex=0,ax=0,ay=0,celebration=0,weather=0){
    if(dt<=0)return;
    this.device.queue.writeBuffer(this.uniform,0,new Float32Array([dt,Math.max(-6,Math.min(6,angularVelocity)),this.liquid.n,vortex,ax,ay,celebration,weather]));
