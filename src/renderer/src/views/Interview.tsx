...
export default function Interview({ projects, projectId, onDone, onCancel, backLabel='Back to the board' }: {
  ...
  return <PlanningTable stage={stage} project={projectOptions.find(row => row.id === draft.project)?.name ?? ''}
    ...
    {stage === 'idea' && <fieldset className="planning-fields" disabled={busy !== null}>
      ...
      <div className="prompt-field-group" data-planning-cue="Keep going. The rough edges are useful."><label htmlFor="planning-goal-idea">Your idea</label><PromptField id="planning-goal-idea" scopeKey={`planning:${draft.project}:idea`} purpose="goal idea" actionsDisabled={busy !== null} className="field planning-seed" aria-label="Your goal idea" data-planning-initial value={draft.seed} maxLength={4000}
        onValueChange={seed => edit({seed})} placeholder="Something to build, a problem to untangle, an idea that won’t leave you alone…" /></div>
      {projectPicker}
    ...
    {stage === 'conversation' && <>
      ...
      {open && open.answer === null && <>
        ...
        <div className="prompt-field-group" data-planning-cue="I’m listening. Specific beats polished."><label htmlFor="planning-answer">Your answer</label><PromptField id="planning-answer" scopeKey={`planning:${record?.id}:${open.at}:${record?.turns.length}:answer`} purpose="planning answer" className="field planning-seed" aria-label="Your planning answer" data-planning-initial key={`${record?.id}:${open.at}:${record?.turns.length}`}
          value={answer} maxLength={4000} disabled={busy !== null} onValueChange={setAnswer}
          onKeyDown={event => {if (!event.nativeEvent.isComposing && (event.metaKey || event.ctrlKey) && event.key === 'Enter') {event.preventDefault(); submitAnswer();}}} /></div>
    ...
    {stage === 'plan' && <>
      ...
      <fieldset className="planning-fields" disabled={busy !== null}>
        ...
        <div className="prompt-field-group" data-planning-cue="What changes when this works?"><label htmlFor="planning-objective">Objective</label><PromptField id="planning-objective" scopeKey={`planning:${draft.project}:${draft.interviewId ?? 'manual'}:objective`} purpose="goal objective" maxLength={12_000} actionsDisabled={busy !== null} className="field" value={draft.objective} onValueChange={objective => edit({objective})} placeholder="What should be different, and why?" /></div>
        <div className="prompt-field-group" data-planning-cue="This is the good part. How will we know?"><label htmlFor="planning-acceptance">Acceptance checks · one per line</label><PromptField id="planning-acceptance" scopeKey={`planning:${draft.project}:${draft.interviewId ?? 'manual'}:acceptance`} purpose="acceptance checks, one per line" actionsDisabled={busy !== null} className="field" value={draft.acceptance} onValueChange={acceptance => edit({acceptance})} placeholder={'Retrying a payment never creates a second charge.\nThe existing checkout tests still pass.'} /></div>
